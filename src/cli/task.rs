...
pub enum TaskCommands {
    /// Add a task to an anchor or subtask to a task
    Add {
        /// Parent ID (anchor or task)
        parent: String,
...
pub fn run(cmd: TaskCommands, output: &Output) -> Result<()> {
    match cmd {
        TaskCommands::Add { parent, title } => add_task(output, &parent, &title),
        TaskCommands::List { anchor } => list_tasks(output, &anchor),
...
fn add_task(output: &Output, parent_str: &str, title: &str) -> Result<()> {
    let project = Project::open_current()?;
    let store = project.task_store();

    // Determine if parent is an anchor or task
    let task_id = if parent_str.contains('.') {
        // Parent is a task - create subtask
        let parent_id: TaskId = parent_str.parse()?;
        ...
        // Parent is an anchor - create top-level task
        let anchor_id: AnchorId = parent_str.parse()?;
...
fn list_tasks(output: &Output, anchor_str: &str) -> Result<()> {
    let project = Project::open_current()?;
    let store = project.task_store();
...
fn show_task(output: &Output, id_str: &str) -> Result<()> {
    ...
    if output.is_json() {
        output.data(&serde_json::json!({
            ...
            "title": task.title,
            "status": task.status,
            "anchor_id": task.anchor_id().to_string(),
